...
class Field:
    ...
    def G(self, x, u):
        _, _, theta = x.ravel()
        delta_rot1, delta_trans, _ = u.ravel()

        theta_hat = theta + delta_rot1
        return np.array([
            [1, 0, -delta_trans * np.sin(theta_hat)],
            [0, 1,  delta_trans * np.cos(theta_hat)],
            [0, 0, 1]
        ])
    ...
    def V(self, x, u):
        ...
        theta_hat = theta + delta_rot1
        return np.array([
            [-delta_trans * np.sin(theta_hat), np.cos(theta_hat), 0],
            [ delta_trans * np.cos(theta_hat), np.sin(theta_hat), 0],
            [1, 0, 1]
        ])
