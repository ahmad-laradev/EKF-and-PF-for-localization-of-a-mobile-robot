...
class ParticleFilter:
    ...
    def update(self, env, u, z, marker_id):
        ...
        for i in range(self.num_particles):
            ...
            new_particles[i, :] = env.forward(particle, noisy_u).ravel()

        # Compute weights
        weights = np.zeros(self.num_particles)
        for i in range(self.num_particles):
            particle = new_particles[i, :]
            expected_z = env.observe(particle, marker_id)
            innovation = minimized_angle(z[0, 0] - expected_z[0, 0])
            innovation = np.array([[innovation]])
            weights[i] = env.likelihood(innovation, self.beta)

        weights += 1e-300  # avoid zero probability
        weights /= weights.sum()
